...
def main():
    ...
    parser.add_argument("--watchdog", type=float, default=10.0, help="워치독 타임아웃 초 (기본: 1.0s)")
    ...
    # ===== (2) 킥 & 홀드 =====
    parser.add_argument("--start-thresh", type=float, default=0.151,
                        help="출발 임계 스로틀(이하에선 본래 안 굴러감). 네 차량 기준 기본 0.15")
    parser.add_argument("--kick", type=float, default=0.15,
    ...
    parser.add_argument("--recv-timeout", type=float, default=0.02,
                        help="recv 타임아웃(초). 디더링을 위해 10~30ms 권장")
    ...
    sock.settimeout(args.recv-timeout if hasattr(args, 'recv-timeout') else args.recv_timeout)  # backward safety
    ...
    last_pdm = time.time()
    accum = 0.0  # 브레젠험 방식 누적
    # 현재 목표 스로틀(수신 없을 때도 유지)
    target_throttle = 0.0
    ...
    print(f"[INFO] 저속 고분해능: thr_scale={args.thr_scale:.2f}, expo={args.expo:.2f}, start_thresh={args.start_thresh:.2f}, "
          f"kick={args.kick:.2f}/{args.kick_ms}ms, pdm_hz={args.pdm_hz:.1f}, deadzone={args.deadzone:.3f}")
    print("[TIP ] ESC를 Training Mode(50% 제한)로 수동 설정하면 추가로 더 안정적인 저속화가 됩니다. (EZ-Set: 빨강 3번 깜박)")
        ...
        while True:
            ...
            if data and len(data) >= PACKET_SIZE:
                ...
                if is_finite(throttle_in) and is_finite(steering_deg):
                    ...
                    thr_shaped = thr_in ** expo          # 0~1, 저속에서 분해능↑
