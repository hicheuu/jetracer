...
# ---------- OLED ----------
disp = Adafruit_SSD1306.SSD1306_128_32(rst=None, i2c_bus=1, gpio=1)  # I2C addr=0x3C (default)
disp.begin()
disp.clear()
disp.display()
...
ina = INA219(i2c, addr=0x42)  # JetRacer 보드에서 배터리 센서 주소
...
def soc_from_voltage(pack_v, cells=2):
    vpc = pack_v / cells
    if vpc >= 4.20: return 100
    if vpc <= 3.00: return 0
